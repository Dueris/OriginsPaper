...
public class Get extends SubCommand {
    ...
    public void perform(CommandSender sender, String[] args) {
        ...
        for (Player p : players) sender.sendMessage(Component.text(Lang.getLocalizedString("command.origin.get.output").replace("%player%", p.getName()).replace("%layer%", args[2]).replace("%origin", OriginPlayer.getOrigin(p, CraftApoli.getLayerFromTag(args[2])).getTag())));
    }
}
