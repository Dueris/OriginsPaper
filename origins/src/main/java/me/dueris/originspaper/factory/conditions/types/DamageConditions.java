...
public class DamageConditions {
	...
	public void registerConditions() {
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("projectile"), (condition, event) -> {
		...
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("name"), (condition, event) -> CraftDamageType.bukkitToMinecraft(event.getDamageSource().getDamageType()).msgId().equalsIgnoreCase(condition.getString("name"))));
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("in_tag"), (condition, event) -> {
		...
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("fire"), (condition, event) -> event.getCause().equals(DamageCause.FIRE)));
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("attacker"), (condition, event) -> {
		...
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("amount"), (condition, event) -> {
		...
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("explosive"), (condition, event) -> event.getDamageSource().getDamageType().equals(DamageType.EXPLOSION) || event.getDamageSource().getDamageType().equals(DamageType.PLAYER_EXPLOSION)));
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("bypasses_armor"), (condition, event) -> OriginsPaper.server.reloadableRegistries().get().registry(net.minecraft.core.registries.Registries.DAMAGE_TYPE).orElseThrow().wrapAsHolder(CraftDamageType.bukkitToMinecraft(event.getDamageSource().getDamageType())).is(DamageTypeTags.BYPASSES_ARMOR)));
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("from_falling"), (condition, event) -> event.getCause().equals(DamageCause.FALL)));
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("out_of_world"), (condition, event) -> event.getCause().equals(DamageCause.WORLD_BORDER)));
		register(new ConditionFactory(OriginsPaper.apoliIdentifier("unblockable"), (condition, event) -> OriginsPaper.server.reloadableRegistries().get().registry(net.minecraft.core.registries.Registries.DAMAGE_TYPE).orElseThrow().wrapAsHolder(CraftDamageType.bukkitToMinecraft(event.getDamageSource().getDamageType())).is(DamageTypeTags.BYPASSES_SHIELD)));
	}
