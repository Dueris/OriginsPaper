...
public class ItemActions {
	...
	public void register() {
		register(new ActionFactory(OriginsPaper.apoliIdentifier("damage"), (action, item) -> item.setDurability((short) (item.getDurability() + action.getNumber("amount").getFloat()))));
		register(new ActionFactory(OriginsPaper.apoliIdentifier("consume"), (action, item) -> {
			int amount = !action.isPresent("amount") ? 1 : action.getNumber("amount").getInt();
			item.setAmount(item.getAmount() - amount);
		}));
		register(new ActionFactory(OriginsPaper.apoliIdentifier("remove_enchantment"), (action, item) -> {
		...
		register(new ActionFactory(OriginsPaper.apoliIdentifier("modify_item_cooldown"), (action, item) -> {
