...
public class EntityGlowPower extends PowerType {
	...
	public static InstanceDefiner buildDefiner() {
		return PowerType.buildDefiner().typedRegistry(OriginsPaper.apoliIdentifier("entity_glow"))
		...
		try {
			if (isActive(player)) {
				for (Entity entity : entities) {
					if (!entityCondition.test(entity)) {
						utils.unsetGlowing(entity.getBukkitEntity(), p);
						continue;
					}
					if (!bientityCondition.test(new Tuple<>(player, entity))) {
						utils.unsetGlowing(entity.getBukkitEntity(), p);
						continue;
					}
