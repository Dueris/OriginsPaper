                ...
                for (Power power : OriginPlayerAccessor.getMultiPowerFileFromType(p, getPowerFile(), layer)) {
                    ...
                    if (Bukkit.getServer().getCurrentTick() % interval != 0) {
                        ...
                        for (Entity entity : getEntitiesWithinRender(p)) {
                            if (ConditionExecutor.testEntity(power, power.get("entity_condition"), (CraftEntity) p)) {
                                if (ConditionExecutor.testBiEntity(power, power.get("bientity_condition"), (CraftEntity) p, (CraftEntity) entity)) {
                                    if (p.canSee(entity)) {
