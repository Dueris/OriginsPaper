            ...
            for (Power power : OriginPlayerAccessor.getMultiPowerFileFromType(p, getPowerFile(), layer)) {
                ...
                for (Entity entity : Bukkit.getServer().getWorld(p.getWorld().getKey()).getEntities()) {
                    if (entity instanceof Player player) {
                        if (ConditionExecutor.testEntity(power, power.get("entity_condition"), (CraftEntity) p) && ConditionExecutor.testEntity(power, power.get("entity_condition"), (CraftEntity) p) && ConditionExecutor.testBiEntity(power, power.get("bientity_condition"), (CraftEntity) p, (CraftEntity) entity)) {
