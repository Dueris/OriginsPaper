...
public class LayerArgumentType implements CustomArgumentType<OriginLayer, NamespacedKey> {
	...
	public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
		return SharedSuggestionProvider.suggestResource(OriginsPaper.getRegistry().retrieve(Registries.LAYER).stream().filter(OriginLayer::isEnabled).map(OriginLayer::getId), builder);
	}
